...
| `RBLN_PROFILER` | `False` (mapped to `VLLM_RBLN_PROFILER`) | Exposed as an RBLN profiler flag. In this repo, it is primarily validated for incompatibility with model parallel execution (TP/DP/EP/PP), where startup raises an error. In installed `rebel`, it is also read in `rebel/core/compiled_model_core.py`. | `vllm_rbln/rbln_envs.py`, `vllm_rbln/platform.py`, `rebel/core/compiled_model_core.py` |
