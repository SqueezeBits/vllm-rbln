...
def _get_qwen3_moe_descriptors():
    return tuple(
        descriptor
        for descriptor in get_registered_patch_descriptors()
        if descriptor.owner_module == "vllm_rbln.patches.models_qwen3_moe"
    )


def test_qwen3_moe_patch_descriptor_is_registry_managed():
    descriptors = _get_qwen3_moe_descriptors()

    assert "vllm_rbln.models.qwen3_moe" not in get_legacy_patch_modules()
...
def test_qwen3_moe_patch_descriptor_updates_target(monkeypatch):
    def original_forward(*args, **kwargs):
        return args, kwargs

    monkeypatch.setattr(Qwen3MoeSparseMoeBlock, "forward", original_forward)
...
def test_qwen3_moe_patch_default_verify_rejects_missing_assignment(monkeypatch):
    descriptor = _get_qwen3_moe_descriptors()[0]
