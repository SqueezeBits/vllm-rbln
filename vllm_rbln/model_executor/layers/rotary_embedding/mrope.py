...
from vllm.model_executor.layers.rotary_embedding.common import (rotate_gptj,
                                                                rotate_neox)
from vllm.model_executor.layers.rotary_embedding.yarn_scaling_rope import yarn_get_mscale
...
def mrope__custom_init__(
    ...
    *,
    ...
    if self.mrope_interleaved:
        idxs = torch.arange(rotary_dim // 2, dtype=torch.int64)
        source_idx = torch.zeros(rotary_dim // 2, dtype=torch.int64)
...
def mrope_forward_oot(
    ...
    if positions_ndim == 2:
        ...
        if self.mrope_interleaved:
            # Graph-friendly interleaved implementation using gather
            # positions is [3, seq_len]
            # self.cos_sin_cache is [max_pos, rotary_dim]
            # We need raw dim/2 components
            
            # Fetch for all 3 position rows at once
            positions_flat = positions.flatten()
            cos_sin_all = self.cos_sin_cache.index_select(0, positions_flat)
            # [3*seq_len, rotary_dim]
            
            cos_all, sin_all = cos_sin_all.chunk(2, dim=-1)
            # [3*seq_len, dim/2]
            
            # Reshape to [3, seq_len, dim/2]
            cos_all = cos_all.view(3, seq_len, -1)
            sin_all = sin_all.view(3, seq_len, -1)
            
            # Prepare indices for gather
            # self.mrope_interleaved_indices: [dim/2]
            # View as [1, 1, dim/2] -> expand to [1, seq_len, dim/2]
            gather_indices = self.mrope_interleaved_indices.view(1, 1, -1).expand(1, seq_len, -1)
            
            cos = cos_all.gather(0, gather_indices).squeeze(0)
            sin = sin_all.gather(0, gather_indices).squeeze(0)
        ...
        # Re-apply neox style or gptj style
        if self.is_neox_style:
            cos = cos.repeat(1, 2)
            sin = sin.repeat(1, 2)
        else:
