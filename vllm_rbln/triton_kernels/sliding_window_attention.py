...
@triton.jit
def sliding_window_attention_naive_prefill_read_only(
    query_base,
    kv_cache_base,
    output_base,
    cache_seq_len_base,
    ...
    for batch_id in tl.static_range(0, NUM_BATCH, 1):
        ...
        # The target/source layer has already written the sliding-window cache.
        # Read it directly and skip the shared layer's K/V insertion and stores.
        k_base = rblib.dynamic_load(k_cache_base_ptr)
        k_base = tl.reshape(k_base, (1, NUM_HEAD, 1, WINDOW_SIZE, HEAD_DIM))
        k_tmp = tl.permute(k_base, (0, 1, 2, 4, 3))
        k = tl.broadcast_to(k_tmp, (1, NUM_HEAD, NUM_GROUP, HEAD_DIM, WINDOW_SIZE))

        qk = tl.dot(q, k)  # (1,h,g,l,d) x (1,h,g,d,w) = (1,h,g,l,w)
        qk_scaled = qk * qk_scale
        window_start = tl.maximum(cache_end - WINDOW_SIZE, 0)
        query_start = cache_start - window_start
        window_qk_scaled = rblib.window_softmax(
            qk_scaled, query_start, window_size=WINDOW_SIZE
        )

        v_base = rblib.dynamic_load(v_cache_base_ptr)
        v = tl.reshape(v_base, (1, NUM_HEAD, 1, WINDOW_SIZE, HEAD_DIM))
        v = tl.broadcast_to(v, (1, NUM_HEAD, NUM_GROUP, WINDOW_SIZE, HEAD_DIM))
        attn_out = tl.dot(window_qk_scaled, v)
        tl.store(output_ptr, attn_out)


@triton.jit
def sliding_window_attention_naive_decode_read_only(
    query_base,
    kv_cache_base,
    output_base,
    cache_seq_len_base,
    ...
    for batch_id in tl.static_range(0, NUM_BATCH, 1):
        ...
        # The target/source layer has already inserted the decode token.
        # Read the cache directly; do not touch the shared layer's K/V.
        k_base = rblib.dynamic_load(k_cache_base_ptr)
        k_base = rblib.nn_pad(k_base, 0.0, WINDOW_AXIS, (0, PAD_SIZE), "constant")
        k_base = tl.reshape(
            k_base, (1, NUM_HEAD, 1, WINDOW_SIZE + PAD_SIZE, HEAD_DIM)
        )
        k_base = tl.permute(k_base, (0, 1, 2, 4, 3))
        k = tl.broadcast_to(
            k_base, (1, NUM_HEAD, NUM_GROUP, HEAD_DIM, WINDOW_SIZE + PAD_SIZE)
        )

        qk = tl.dot(q, k)  # (1,h,g,l,d) x (1,h,g,d,w+pad) = (1,h,g,l,w+pad)
        qk_scaled = qk * qk_scale
        window_start = tl.maximum(cache_end - WINDOW_SIZE, 0)
        query_start = cache_start - window_start
        window_qk_scaled = rblib.window_softmax(
        ...
        v_base = rblib.dynamic_load(v_cache_base_ptr)
        v_base = rblib.nn_pad(v_base, 0.0, WINDOW_AXIS, (0, PAD_SIZE), "constant")
        v = tl.reshape(v_base, (1, NUM_HEAD, 1, WINDOW_SIZE + PAD_SIZE, HEAD_DIM))
        v = tl.broadcast_to(
            v, (1, NUM_HEAD, NUM_GROUP, WINDOW_SIZE + PAD_SIZE, HEAD_DIM)
        )
        attn_out = tl.dot(window_qk_scaled, v)
...
@triton_op(
    "rbln_triton_ops::sliding_window_attention_naive_prefill_read_only",
    mutates_args=(),
)
def _(
    query: torch.Tensor,
    ...
    warmup(sliding_window_attention_naive_prefill_read_only, *params)

    return output.to(original_dtype)


@register_fake("rbln_triton_ops::sliding_window_attention_naive_prefill_read_only")
def _(
    query: torch.Tensor,
    kv_cache: torch.Tensor,
    cache_seq_len: torch.Tensor,
...
@triton_op(
    "rbln_triton_ops::sliding_window_attention_naive_decode_read_only",
    mutates_args=(),
)
def _(
    query: torch.Tensor,
    ...
    warmup(sliding_window_attention_naive_decode_read_only, *params)

    return output.to(original_dtype)


@register_fake("rbln_triton_ops::sliding_window_attention_naive_decode_read_only")
def _(
    query: torch.Tensor,
    kv_cache: torch.Tensor,
    cache_seq_len: torch.Tensor,
